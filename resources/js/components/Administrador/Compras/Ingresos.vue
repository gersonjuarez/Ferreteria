...
>
    ...
    <v-container>
      <v-row>
        ...
        :required="required"
         :rules="[v => !this.required || !!v || 'Proveedor Requerido.']"
              :disabled=" desactivar"
...
export default {
  ...
  data(){
          ...
          precio_minimo:0,
              ...
              busqueda:null,
              select: "",
              idProveedor:null, //Registro de Compra
              tipo_comprobante:'',
              serie_comprobante:'',
              num_comprobante:'',
              impuesto:null,
  ...
  watch:{
    ...
    busqueda(val) {
      ...
      if(me.busqueda_compra.length){          
        ...
        me.aux2=null;
                ...
                var url= '/ingreso?page=' + me.pagination_ingreso.current_page+ '&buscar='+ me.busqueda_compra;
                axios.get(url).then(function (response) {
                        me.Ingresos=[];

                    var respuesta= response.data;
                    console.log('datos: '+JSON.stringify(respuesta))
                    me.Ingresos = respuesta.ingresos.data;
   ...
   listarProducto() {
    ...
    selectProveedor(valor){
                ...
                var url= '/proveedor/selectProveedor?filtro='+valor;
                axios.get(url).then(function (response) {
                  me.items=[];
                    let respuesta = response.data;

                    me.items= respuesta.proveedores
